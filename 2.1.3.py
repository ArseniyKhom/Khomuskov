...
class Report:
    ...
    def generate_image(self):
        figure, ((valX_1, valX_2), (valX_3, valX_4)) = plt.subplots(nrows=2, ncols=2)

        rack1 = self.currency_rack1_method(valX_1)
        rack2 = self.currency_rack2_method(valX_1)
        valX_1.set_title('Уровень зарплат по годам', fontdict={'fontsize': 8})
        valX_1.grid(axis='y')
        valX_1.legend((rack1[0], rack2[0]), ('средняя з/п', 'з/п ' + self.vacancy_name.lower()), prop={'size': 8})
        valX_1.set_xticks(np.array(list(self.statistics1.keys())) - 0.2, list(self.statistics1.keys()), rotation=90)
        valX_1.xaxis.set_tick_params(labelsize=8)
        valX_1.yaxis.set_tick_params(labelsize=8)

        valX_2.set_title('Количество вакансий по годам', fontdict={'fontsize': 8})
        rack1 = self.vacancy_rack1_method(valX_2)
        rack2 = self.vacancy_rack2_method(valX_2)
        valX_2.legend((rack1[0], rack2[0]), ('Количество вакансий', 'Количество вакансий\n' + self.vacancy_name.lower()), prop={'size': 8})
        valX_2.set_xticks(np.array(list(self.statistics2.keys())) - 0.2, list(self.statistics2.keys()), rotation=90)
        valX_2.grid(axis='y')
        valX_2.xaxis.set_tick_params(labelsize=8)
        valX_2.yaxis.set_tick_params(labelsize=8)

        valX_3.set_title('Уровень зарплат по городам', fontdict={'fontsize': 8})
        valX_3.barh(list([str(a).replace(' ', '\n').replace('-', '-\n') for a in reversed(list(self.statistics5.keys()))]), list(reversed(list(self.statistics5.values()))), color='blue', height=0.5, align='center')
        valX_3.yaxis.set_tick_params(labelsize=6)
        valX_3.xaxis.set_tick_params(labelsize=8)
        valX_3.grid(axis='x')

        valX_4.set_title('Доля вакансий по городам', fontdict={'fontsize': 8})
        other = 1 - sum([meaning for meaning in self.statistics6.values()])
        color_arr = ['blue', 'orange', 'green', 'red', 'violet', 'brown', 'pink', 'gray', 'yellow', 'turquoise', 'salmon', 'purple']
        valX_4.pie(list(self.statistics6.values()) + [other], labels=list(self.statistics6.keys()) + ['Другие'], textprops={'fontsize': 6}, colors=color_arr)

        plt.tight_layout()
        plt.savefig('graph.png')

    def currency_rack1_method(self, valX_1):
        list_result = list(self.statistics1.keys())
        result = valX_1.bar(np.array(list_result) - 0.4, self.statistics1.values(), width=0.4)
        return result

    def currency_rack2_method(self, valX_1):
        list_result = list(self.statistics1.keys())
        result = valX_1.bar(np.array(list_result), self.statistics3.values(), width=0.4)
        return result


    def vacancy_rack1_method(self, valX_2):
        list_result = list(self.statistics2.keys())
        result = valX_2.bar(np.array(list_result) - 0.4, self.statistics2.values(), width=0.4)
        return result

    def vacancy_rack2_method(self, valX_2):
        list_result = list(self.statistics2.keys())
        result = valX_2.bar(np.array(list_result), self.statistics4.values(), width=0.4)
        return result
    ...
    def generate_pdf(self):
        ...
        pattern = env.get_template("pdf_template.html")
        statistics = []
        for year in self.statistics1.keys():
            statistics.append([year, self.statistics1[year], self.statistics2[year], self.statistics3[year], self.statistics4[year]])
        for clue in self.statistics6:
            self.statistics6[clue] = round(self.statistics6[clue] * 100, 2)
        pdf_pattern = pattern.render({'name': self.vacancy_name, 'path': '{0}/{1}'.format(pathlib.Path(__file__).parent.resolve(), 'graph.png'), 'stats': statistics, 'stats5': self.statistics5, 'stats6': self.statistics6})
        config = pdfkit.configuration(wkhtmltopdf=r'C:\Program Files (x86)\wkhtmltopdf\bin\wkhtmltopdf.exe')
        pdfkit.from_string(pdf_pattern, 'report.pdf', configuration=config, options={"enable-local-file-access": ""})
