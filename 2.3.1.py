...
class DataSet:
    ...
    def __init__(self, file_name):
        """Инициализирует объект DataSet, обработка данных
...
class Vacancy:
    ...
    area_name (str): Город, предоставляющий вакансию
    ...
    def __init__(self, one_vac):
        ...
        Args:
        one_vac (dict): Словарь, содержащий данные об одной вакансии
        """
    ...
    def fill_data_dicts(self, salary_dicts, count_dicts, vacancy):
        """Заполняет словари значениями, работает только со словарями класса StaticInfo
...
class StaticSalary:
    """Класс для получения статистики по зарплатам

    Attributes:
    salaries (dict): Словарь, содержащий уровень зарплат по годам
    vacancy_salaries (dict): Словарь, содержащий уровня зарплат по годам для выбранной профессии
    salaries_areas (dict): Словарь, содержащий уровень зарплат по городам (10 значений)
    ...
    def __init__(self):
        """Инициализирует объект StaticSalary, подготовка словарей для работы с ними
        """
    ...
    def check_len_dic(dic, vacancy=""):
        ...
        Args:
        dic (dict): Словарь, нуждающийся в проверке
        vacancy (str): Название введенной профессии

        Returns:
        new_dict (dict): Отформатированный словарь
        """
        new_dic = {}
...
class StaticCount:
    """Класс для получения статистики по количеству вакансий

    Attributes:
    vac_count (dict): Словарь, содержащий количество вакансий по годам
    vacancy_count (dict): Словарь, содержащий количество вакансий по годам для выбранной профессии
    vacancies_areas (dict): Словарь, содержащий часть вакансий по городам (10)
    """
    vac_count: dict = {}
    vacancy_count: dict = {}
    ...
    def __init__(self):
        """Инициализирует объект StaticCount, подготовка словарей для работы с ними
        """
        for i in range(2007, 2023):
            self.vac_count[i] = 0
    ...
    def check_int_dic(dic, vacancy=""):
        ...
        Returns:
        new_dict (dict): Словарь после проверки
        """
        new_dic = {}
        for name, item in dic.items():
            if item != 0:
...
class ResultStatic:
    ...
    def check_one_percent(self, salaries, counts):
        """Вычисляет процент вакансий из общего количества в словарях по городам и оставляет словари, удовлетворяющий условию
        """
    ...
    def sort_dict(dictionary):
        ...
        Args:
        dictionary (dict): Словарь, нуждающийся в сортировке

        Returns:
        sorted_dict (dict): Словарь после сортировки
        """
    ...
    def print_result(self, vacancy, salaries, counts):
        """Выводит словари и формирует списки по ним
...
class Report:
    ...
    def __init__(self, vacancy_name, list_by_year, list_by_area, others):
        """Инициализация объекта, создание экземпляров
    ...
    def generate_pdf(vacancy_name, list_by_year, list_by_area):
        """Генерирует файл с необходимой статистикой
    ...
    def generate_image(vacancy_name, list_by_year, list_by_area, others):
        """Генерирует изображение с необходимой статистикой
    ...
    def generate_excel(self, vacancy_name, list_by_year, list_by_area):
        """Генерирует таблицу с необходимой статистикой
    ...
    def set_width(self, thins):
        """Устанавливает необходимые размеры
        """
        dims = {}
